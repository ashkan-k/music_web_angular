...
export class SettingsListComponent implements OnInit {
  ...
  async GetSettings(offset: number = 0) {
    if (this.search){
    ...
    this.http.get(`${this.global_properties.BASE_YRL}/api/admin/settings/?search=${this.search}&offset=${offset}&limit=${this.global_properties.PAGINATION_NUMBER}`,{
      headers: this.global_properties.headers
    })
      .toPromise()
      .then(c => {
        // @ts-ignore
        this.settings = c['data'];
        // @ts-ignore
  ...
  DeleteUser(id: number) {
    ...
    this.http.delete(`${this.global_properties.BASE_YRL}/api/admin/settings/${id}/`,{
      headers: this.global_properties.headers
    })
      .toPromise()
      .then(c => {
  ...
  Get(id: number) {
    ...
    this.http.get(`${this.global_properties.BASE_YRL}/api/admin/settings/${id}`,{
      headers: this.global_properties.headers
    })
      .toPromise()
      .then(c => {
